...
def count_vehicle(box_id, img):
    ...
    if sum(down_list) > 5:
        cv2.putText(img, "Limit reached", (x, y - 20), cv2.FONT_HERSHEY_SIMPLEX, font_size, (0, 0, 255), font_thickness)
...
def realTime():
    ...
    while True:
        ...
        # Show the message "Limit reached" if the total count of vehicles in the down section is more than 5
        if total_count == 0:
            cv2.putText(img, "Limit reached", (260, 20), cv2.FONT_HERSHEY_SIMPLEX,
                        font_size, (255, 0, 0), font_thickness)

        # Show the frames
        k = cv2.imshow('Output', img)
